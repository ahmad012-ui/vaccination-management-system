  ...
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-radius-lg fixed-start ms-2 bg-white my-2" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active bg-gradient-dark text-white" href="../pages/notifications.php">
                    ...
                    <div class="table-responsive p-0">
                      <table class="table align-items-center mb-0">
                        ...
                        <tbody>
                          <?php
                            $notif_query = "
                              SELECT c.name AS child_name, u.name AS parent_name, ar.status, ar.created_at
                              FROM appointment_requests ar
                              JOIN children c ON ar.child_id = c.child_id
                            ...
                            if (mysqli_num_rows($notif_result) > 0) {
                              while ($row = mysqli_fetch_assoc($notif_result)) {
                                $msg = "{$row['parent_name']} booked an appointment for {$row['child_name']}.";
                                $badge = $row['status'] === 'Approved' ? 'success' : ($row['status'] === 'Pending' ? 'warning' : 'secondary');
                                echo "
                                  <tr>
                                    <td class='ps-4 text-sm'>{$msg}</td>
                                    <td class='text-sm'>" . date("d M Y, h:i A", strtotime($row['created_at'])) . "</td>
                                    <td><span class='badge bg-gradient-{$badge}'>{$row['status']}</span></td>
                                  </tr>
                                ";
                              }
                            } else {
                              echo "<tr><td colspan='3' class='text-center text-muted'>No notifications yet.</td></tr>";
                            }
                          ?>
                        </tbody>
                      </table>
