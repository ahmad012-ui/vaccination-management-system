...
<body class="g-sidenav-show  bg-gray-100">
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-radius-lg fixed-start ms-2  bg-white my-2" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active bg-gradient-dark  text-white" href="../pages/patient.php">
                ...
                <table class="table align-items-center mb-0">
                  <thead class="text-center">
                    <tr>
                      ...
                      <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7">Registered By</th>
                      <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7">Registered At</th>
  ...
  u.email,
  COALESCE(ar.status, 'Not Vaccinated') AS vaccination_status
FROM children c
JOIN users u ON c.parent_id = u.user_id
LEFT JOIN (
    SELECT child_id, MAX(status) AS status
    FROM appointment_requests
    GROUP BY child_id
) ar ON c.child_id = ar.child_id
...
";
  ...
  <td><?php echo htmlspecialchars(
      $row["parent_name"]
  ); ?> (<?php echo htmlspecialchars($row["email"]); ?>)</td>
  <td><?php echo htmlspecialchars(
      date("d M Y", strtotime($row["created_at"]))
  ); ?></td>
  <td><span class="badge badge-sm <?php echo $badge; ?>"><?php echo ucfirst(
    $status
); ?></span></td>
</tr>
<?php
