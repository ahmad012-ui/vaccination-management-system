    ...
    <div class="container-fluid py-2">
      <div class="row">
        <div class="ms-3">
          ...
          <p class="mb-4">
            Check patients, appointments, hospitals and vaccination stats at a glance.
          </p>
        </div>
          ...
          <div class="card">
            <div class="card-header p-2 ps-3">
              <div class="d-flex justify-content-between">
                <div>
                  <h5 class="mb-0 font-weight-bolder">
                    <!-- Get the total number of appointments in a day from the database -->
                    <?php
                      $result = mysqli_query($conn, "SELECT COUNT(*) AS total FROM appointment_requests WHERE DATE(preferred_date) = CURDATE()");
                      $row = mysqli_fetch_assoc($result);
                      echo $row['total'];
                    ?>
                  </h5>
      ...
      <div class="row mt-4 mb-4">
        ...
        <!-- appointments in details (per day) -->
            ...
            <div class="card-header pb-0">
              ...
              <p class="text-sm">
                ...
                <span class="font-weight-bold">
                  <?php
                  $month = date('m');
                  $count = mysqli_query($conn, "SELECT COUNT(*) as total FROM children WHERE MONTH(created_at) = $month");
                    ...
                    <table class="table align-items-center mb-0">
                      <thead>
                        <tr>
                          ...
                          <th class="text-uppercase text-xs font-weight-bolder opacity-7 ps-2">Age</th>
                          <th class="text-uppercase text-xs font-weight-bolder opacity-7 ps-2">Gender</th>
                        </tr>
                      </thead>
                      <tbody>
                        <?php
                          ...
                          $result = mysqli_query(
                            $conn,
                            "SELECT name, TIMESTAMPDIFF(YEAR, dob, CURDATE()) AS age, gender FROM children WHERE MONTH(created_at) = $month ORDER BY created_at DESC LIMIT 6"
                          );

                          if (mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                              echo "<tr>
                                      <td><p class='text-sm font-weight-bold mb-0'>{$row['name']}</p></td>
                                      <td><p class='text-sm mb-0'>{$row['age']}</p></td>
                                      <td><p class='text-sm mb-0'>{$row['gender']}</p></td>
                                    </tr>";
                            }
                          } else {
                            echo "<tr><td colspan='3'><p class='text-center text-muted'>No new children this month.</p></td></tr>";
                          }
                        ?>
                      </tbody>
                      </tbody>
